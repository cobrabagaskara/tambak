(function() {
    ...
    // =========================
    // TEMPLATE SKRINING INDRA PENDENGARAN
    // =========================
    const pendengaranTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningindrapendengaran/create/",
            skrining: "indra_pendengaran",
            judul: "Skrining Indra Pendengaran",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            { type: "setRadio", name: "curiga_telinga_kiri", value: "Tidak" },
            { type: "setRadio", name: "pemeriksaan_telinga_kiri", value: "Tidak" },
            { type: "setRadio", name: "omsk_telinga_kiri", value: "Tidak" },
            { type: "setRadio", name: "presbikusis_telinga_kiri", value: "Tidak" },
            { type: "setRadio", name: "curiga_telinga_kanan", value: "Tidak" },
            { type: "setRadio", name: "pemeriksaan_telinga_kanan", value: "Tidak" },
            { type: "setRadio", name: "omsk_telinga_kanan", value: "Tidak" },
            { type: "setRadio", name: "presbikusis_telinga_kanan", value: "Tidak" },
            { type: "setRadio", name: "curiga_rujuk", value: "Tidak Rujuk" },
            { type: "setRadio", name: "pemeriksaan_rujuk", value: "Tidak Rujuk" },
            { type: "setRadio", name: "omsk_rujuk", value: "Tidak Rujuk" },
            { type: "setRadio", name: "bisikan_telinga_kiri", value: "275727004" },
            { type: "setRadio", name: "bisikan_telinga_kanan", value: "275727004" },
            { type: "setRadio", name: "otoskop_kiri", value: "0" },
            { type: "setRadio", name: "otoskop_kanan", value: "0" }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningindrapendengaran\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Indra Pendengaran berlaku untuk semua pasien`);

        // Langsung fill form
        fillPendengaranForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('pendengaranProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'pendengaranProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('pendengaranProgress');
        if (progressBar) {
    ...
    // =========================
    function fillPendengaranForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Indra Pendengaran...');

        // Tampilkan progress
        showProgress('Mengisi form Indra Pendengaran...');

        // Auto-fill sesuai template
        pendengaranTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === pendengaranTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL INDRA PENDENGARAN SELESAI
            </h2>
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                • 15 field telah terisi otomatis<br>
                • Semua hasil pemeriksaan: <span style="color: #27ae60; font-weight: bold;">"Tidak"</span><br>
                • Bisikan kiri & kanan: <span style="color: #9b59b6; font-weight: bold;">"275727004"</span><br>
                • Otoskop kiri & kanan: <span style="color: #9b59b6; font-weight: bold;">"0"</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Indra Pendengaran telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('pendengaranProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Indra Pendengaran telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Indra Pendengaran loaded (15 field, tanpa kondisi)');
