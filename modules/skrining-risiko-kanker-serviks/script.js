(function() {
    ...
    // =========================
    // TEMPLATE SKRINING RISIKO KANKER SERVIKS
    // =========================
    const serviksTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/risikokankerserviks/create/",
            skrining: "risiko_kanker_serviks",
            judul: "Skrining Risiko Kanker Serviks",
            conditions: {
                gender: "Perempuan",
                minAge: null,
                maxAge: null,
                notes: "Template untuk Perempuan semua usia"
            }
        },
        actions: [
    ...
    function checkGenderAndFillForm() {
        ...
        if (gender !== "Perempuan") {
            ...
            console.log(`🚫 Skrining Risiko Kanker Serviks tidak berlaku untuk ${gender}`);
            showErrorPopup(
                `Skrining Risiko Kanker Serviks hanya untuk Perempuan`,
        ...
        console.log('✅ Skrining Risiko Kanker Serviks berlaku untuk pasien ini');
        fillServiksForm(patientData);
    }
    ...
    function showErrorPopup(title, message) {
        ...
        popup.innerHTML = `
            ...
            <div style="margin-top: 20px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #7f8c8d;">
                ℹ️ Skrining Risiko Kanker Serviks khusus untuk pasien perempuan.
            </div>
        `;
    ...
    function showProgress(message) {
        const existing = document.getElementById('serviksProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'serviksProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('serviksProgress');
        if (progressBar) {
    ...
    // =========================
    function fillServiksForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Risiko Kanker Serviks...');

        // Tampilkan progress
        showProgress('Mengisi form Risiko Kanker Serviks...');

        // Auto-fill sesuai template
        serviksTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === serviksTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL RISIKO KANKER SERVIKS SELESAI
            </h2>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('serviksProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Risiko Kanker Serviks telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Risiko Kanker Serviks loaded');
