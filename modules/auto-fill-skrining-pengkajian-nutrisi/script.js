(function() {
    ...
    // =========================
    // TEMPLATE SKRINING PENGKAJIAN NUTRISI (SHORT-FORM MNA)
    // =========================
    const mnaTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningpengkajiannutrisi/create/",
            skrining: "pengkajian_nutrisi_mna",
            judul: "Formulir Skrining Pengkajian Nutrisi (Short-Form MNA)",
            conditions: {
                gender: "Semua",
                minAge: 60,
                maxAge: null,
                notes: "Template Short-Form MNA untuk lansia usia 60+ tahun"
            }
        },
        actions: [
            { type: "setRadio", name: "penurunan_asupan_makanan", value: "1" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningpengkajiannutrisi\/create\/(\d+)/);
    ...
    function checkAgeAndFillForm() {
        ...
        if (usia < 60) {
            ...
            console.log(`🚫 Skrining Pengkajian Nutrisi tidak berlaku untuk usia ${usia} tahun`);
            showErrorPopup(
                `Skrining Pengkajian Nutrisi (Short-Form MNA) hanya untuk Lansia (60+ tahun)`,
        ...
        console.log(`✅ Skrining Pengkajian Nutrisi berlaku untuk pasien ini (${usia} tahun)`);
        fillMNAForm(patientData);
    }
    ...
    function showErrorPopup(title, message) {
        ...
        popup.innerHTML = `
            ...
            <div style="margin-top: 20px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #7f8c8d;">
                ℹ️ Skrining Pengkajian Nutrisi (Short-Form MNA) khusus untuk lansia usia 60 tahun ke atas.
            </div>
        `;
    ...
    function showProgress(message) {
        const existing = document.getElementById('mnaProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'mnaProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('mnaProgress');
        if (progressBar) {
    ...
    // FILL FORM
    // =========================
    function fillMNAForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Pengkajian Nutrisi (Short-Form MNA)...');

        // Tampilkan progress
        showProgress('Mengisi form Pengkajian Nutrisi...');

        // Auto-fill sesuai template
        mnaTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === mnaTemplate.actions.length - 1) {
                    setTimeout(() => {
                        hideProgress();
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL PENGKAJIAN NUTRISI (SHORT-FORM MNA) SELESAI
            </h2>
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Pengkajian Nutrisi (Short-Form MNA) telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('mnaProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Pengkajian Nutrisi telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Pengkajian Nutrisi (Short-Form MNA) loaded');
