(function() {
    ...
    // =========================
    // TEMPLATE SKRINING GIGI DEWASA
    // =========================
    const gigiDewasaTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningkesehatangigidewasa/create/",
            skrining: "gigi_dewasa",
            judul: "Skrining Gigi Dewasa",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            ...
            { type: "setRadio", name: "SkriningGigi[gigi_bungsu]", value: "Ya" },
            { type: "setRadio", name: "SkriningGigi[gigi_hilang]", value: "Tidak" },
            { type: "setRadio", name: "SkriningGigi[gigi_berlubang]", value: "Ya" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningkesehatangigidewasa\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Gigi Dewasa berlaku untuk semua pasien`);

        // Langsung fill form
        fillGigiDewasaForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('gigiDewasaProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'gigiDewasaProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('gigiDewasaProgress');
        if (progressBar) {
    ...
    // =========================
    function fillGigiDewasaForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Gigi Dewasa...');

        // Tampilkan progress
        showProgress('Mengisi form Gigi Dewasa...');

        // Auto-fill sesuai template
        gigiDewasaTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === gigiDewasaTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                • 7 field telah terisi otomatis<br>
                • Gigi Bungsu: <span style="color: #27ae60; font-weight: bold;">"Ya"</span><br>
                • Gigi Berlubang: <span style="color: #27ae60; font-weight: bold;">"Ya"</span><br>
                • Karies Gigi: <span style="color: #9b59b6; font-weight: bold;">"1"</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Gigi Dewasa telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('gigiDewasaProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Gigi Dewasa telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Gigi Dewasa loaded (7 field, tanpa kondisi)');
