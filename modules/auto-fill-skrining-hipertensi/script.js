(function() {
    ...
    // =========================
    // TEMPLATE SKRINING HIPERTENSI
    // =========================
    const hipertensiTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skrininghipertensi/create/",
            skrining: "skrining_hipertensi",
            judul: "Skrining Hipertensi",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            { type: "setRadio", name: "riwayat_pribadi", value: "Tidak" },
            { type: "setRadio", name: "riwayat_keluarga", value: "Tidak" },
            { type: "setRadio", name: "riwayat_merokok", value: "Tidak" },
            { type: "setRadio", name: "riwayat_alkohol", value: "Tidak" },
            { type: "setRadio", name: "makan_asin", value: "Ya" },
            { type: "setRadio", name: "aktifitas_fisik", value: "Ya" },
            { type: "setRadio", name: "istirahat_cukup", value: "Ya" },
            { type: "setRadio", name: "kurang_buah_sayur", value: "Tidak" }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skrininghipertensi\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Hipertensi berlaku untuk semua pasien`);

        // Langsung fill form
        fillHipertensiForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('hipertensiProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'hipertensiProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('hipertensiProgress');
        if (progressBar) {
    ...
    // =========================
    function fillHipertensiForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Hipertensi...');

        // Tampilkan progress
        showProgress('Mengisi form Hipertensi...');

        // Auto-fill sesuai template
        hipertensiTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === hipertensiTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Hipertensi telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('hipertensiProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Hipertensi telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Hipertensi loaded');
