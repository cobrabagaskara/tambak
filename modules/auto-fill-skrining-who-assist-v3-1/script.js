(function() {
    ...
    // =========================
    // TEMPLATE SKRINING WHO - ASSIST V3.1
    // =========================
    const assistTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningwho/create/",
            skrining: "who_assist_v31",
            judul: "Skrining WHO - ASSIST V3.1",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            { type: "setRadio", name: "SkriningWho[q_1_a]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_b]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_c]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_d]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_e]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_f]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_g]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_h]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_i]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_1_j]", value: "Tidak pernah" },
            { type: "setRadio", name: "SkriningWho[q_8]", value: "Tidak pernah" }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningwho\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining WHO - ASSIST V3.1 berlaku untuk semua pasien`);

        // Langsung fill form
        fillASSISTForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('assistProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'assistProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('assistProgress');
        if (progressBar) {
    ...
    // =========================
    function fillASSISTForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining WHO - ASSIST V3.1...');

        // Tampilkan progress
        showProgress('Mengisi form ASSIST V3.1...');

        // Auto-fill sesuai template
        assistTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === assistTemplate.actions.length - 1) {
                    setTimeout(() => {
            ...
            }, index * 160); // Delay 160ms antar action
        });
    }
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL WHO - ASSIST V3.1 SELESAI
            </h2>
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                • 11 field telah terisi otomatis<br>
                • Semua jawaban: <span style="color: #9b59b6; font-weight: bold;">"Tidak pernah"</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining WHO - ASSIST V3.1 telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('assistProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill WHO - ASSIST V3.1 telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining WHO - ASSIST V3.1 loaded (11 field, tanpa kondisi)');
