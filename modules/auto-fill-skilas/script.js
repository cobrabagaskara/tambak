(function() {
    ...
    // TEMPLATE SKILAS
    // =========================
    const skilasTemplate = {
        metadata: {
            url: "https://cirebon.epuskesmas.id/skriningskilas/create/",
            skrining: "skilas",
            judul: "Formulir Skrining Lansia Sederhana (SKILAS)",
            conditions: {
                minAge: 60,
                maxAge: null,
                notes: "Template SKILAS untuk lansia usia 60+ tahun"
            }
        },
        actions: [
            { type: "setRadio", name: "malnutrisi_badanberkurang", value: "0" },
            { type: "setRadio", name: "malnutrisi_makan", value: "0" },
            { type: "setRadio", name: "penglihatan_masalahmata", value: "1" },
            { type: "setRadio", name: "depresi_terganggu", value: "0" },
            { type: "setRadio", name: "depresi_minat", value: "0" }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningskilas\/create\/(\d+)/);
    ...
    function checkAgeAndFillForm() {
        ...
        if (!patientId) {
            ...
            showAgeErrorPopup('ID pasien tidak terdeteksi', 'Silakan buka halaman data pasien terlebih dahulu');
            return;
        }
        ...
        if (usia < 60) {
            ...
            console.log(`🚫 Skrining SKILAS tidak berlaku untuk usia ${usia} tahun`);
            showAgeErrorPopup(
                `Skrining SKILAS hanya untuk Lansia (60+ tahun)`,
        ...
        console.log(`✅ Skrining SKILAS berlaku untuk pasien ini (${usia} tahun)`);
        fillSkilasForm(patientData);
    }

    // =========================
    // POPUP ERROR (USIA < 60)
    // =========================
    function showAgeErrorPopup(title, message) {
        ...
        popup.innerHTML = `
            ...
            <div style="margin-top: 20px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #7f8c8d;">
                ℹ️ Skrining SKILAS khusus untuk lansia usia 60 tahun ke atas.
            </div>
        `;
    ...
    function showProgress(message) {
        const existing = document.getElementById('skilasProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'skilasProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('skilasProgress');
        if (progressBar) {
    ...
    // FILL FORM
    // =========================
    function fillSkilasForm(patientData) {
        console.log('🚀 Memulai auto-fill form SKILAS...');

        // Tampilkan progress
        showProgress('Mengisi form SKILAS...');

        // Auto-fill sesuai template
        skilasTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === skilasTemplate.actions.length - 1) {
                    setTimeout(() => {
                        hideProgress();
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form SKILAS telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('skilasProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill SKILAS telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill SKILAS loaded');
