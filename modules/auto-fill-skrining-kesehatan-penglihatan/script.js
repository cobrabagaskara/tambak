(function() {
    ...
    // =========================
    const penglihatanTemplates = {
        // Template untuk usia < 18 tahun
        '<18': {
            meta: {
                url: "https://cirebon.epuskesmas.id/skriningkesehatanpenglihatan/create/",
                skrining: "penglihatan_<18",
                judul: "Formulir Skrining Kesehatan Penglihatan",
                ageRange: "< 18 tahun",
                notes: "Template untuk usia kurang dari 18 tahun"
            },
            actions: [
                { type: "setRadio", name: "mata_luar", value: "1" },
                { type: "setRadio", name: "jenis_pemeriksaan_mata", value: "E tumbling" },
                { type: "setRadio", name: "tajam_kiri", value: "1" },
                { type: "setRadio", name: "tajam_kanan", value: "1" },
                { type: "setRadio", name: "pemeriksaan_visus_rabun_dekat_kiri", value: "0" },
                { type: "setRadio", name: "pemeriksaan_visus_rabun_dekat_kanan", value: "0" },
                { type: "setRadio", name: "buta_warna_kiri", value: "0" },
                { type: "setRadio", name: "buta_warna_kanan", value: "0" },
                { type: "setRadio", name: "kacamata", value: "0" },
                { type: "setRadio", name: "gangguan_refraksi_kiri", value: "0" },
                { type: "setRadio", name: "gangguan_refraksi_kanan", value: "0" },
                { type: "setRadio", name: "katarak_kiri", value: "0" },
                { type: "setRadio", name: "katarak_kanan", value: "0" },
                { type: "setRadio", name: "pinhole_kiri", value: "6/6" },
                { type: "setRadio", name: "pinhole_kanan", value: "6/6" },
                { type: "setRadio", name: "pemeriksaan_pupil_kiri", value: "1" },
                { type: "setRadio", name: "pemeriksaan_pupil_kanan", value: "1" },
                { type: "setRadio", name: "rujuk_rs_dua", value: "0" }
            ]
        },
        ...
        '>=18': {
            meta: {
                ...
                judul: "Formulir Skrining Kesehatan Penglihatan",
                ageRange: "≥ 18 tahun",
                notes: "Template untuk usia 18 tahun ke atas"
            },
            actions: [
                { type: "setRadio", name: "mata_luar", value: "1" },
                { type: "setRadio", name: "jenis_pemeriksaan_mata", value: "Uncorrected Snellen Chart" },
                { type: "setRadio", name: "tajam_kiri", value: "1" },
                { type: "setRadio", name: "tajam_kanan", value: "1" },
                { type: "setRadio", name: "pemeriksaan_visus_rabun_dekat_kiri", value: "0" },
                { type: "setRadio", name: "pemeriksaan_visus_rabun_dekat_kanan", value: "0" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningkesehatanpenglihatan\/create\/(\d+)/);
    ...
    function checkAgeAndSelectTemplate() {
        ...
        if (isNaN(usia) || usia < 0) {
            ...
            showErrorPopup(
                ...
                `Pasien: ${nama}\nUsia: ${patientData.usia} tahun\n\nSistem tidak dapat memproses skrining karena data usia tidak valid.`
            );
            return;
        }

        // Tentukan template berdasarkan usia
        let templateKey;
        let ageRange;
        ...
        const selectedTemplate = penglihatanTemplates[templateKey];
        ...
        console.log(`✅ Skrining Kesehatan Penglihatan berlaku untuk pasien ini (${usia} tahun)`);

        // Langsung fill form dengan template yang dipilih
        fillPenglihatanForm(patientData, selectedTemplate, ageRange);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('penglihatanProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'penglihatanProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('penglihatanProgress');
        if (progressBar) {
    ...
    // =========================
    function fillPenglihatanForm(patientData, template, ageRange) {
        console.log(`🚀 Memulai auto-fill Skrining Kesehatan Penglihatan (${ageRange})...`);

        // Tampilkan progress
        showProgress(`Mengisi form Penglihatan (${ageRange})...`);
    ...
    function showCompletionPopup(patientData, ageRange) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL PENGELIHATAN SELESAI
            </h2>
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                <strong>Kategori Usia:  ${ageRange}</strong>
                
            </div>
 
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Kesehatan Penglihatan telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('penglihatanProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Penglihatan telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Kesehatan Penglihatan loaded (2 templates: <18, >=18)');
