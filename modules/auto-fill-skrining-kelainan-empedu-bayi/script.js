(function() {
    ...
    // =========================
    // TEMPLATES BERDASARKAN USIA DALAM BULAN
    // =========================
    const empeduBayiTemplates = {
        // Template untuk usia < 1 bulan
        '<1bulan': {
            meta: {
                url: "https://cirebon.epuskesmas.id/skriningkelainanempedubayi/create/",
                skrining: "empedu_bayi_<1bulan",
                judul: "Skrining Kelainan Empedu Bayi",
                ageRange: "< 1 bulan",
                notes: "Template untuk bayi kurang dari 1 bulan"
            },
            actions: [
                { type: "setRadio", name: "skrining_jawaban_id", value: "48" }
            ]
        },
        ...
        '>1bulan': {
            meta: {
                ...
                judul: "Skrining Kelainan Empedu Bayi",
                ageRange: "> 1 bulan",
                notes: "Template untuk bayi lebih dari 1 bulan"
            },
            actions: [
                { type: "setRadio", name: "skrining_jawaban_id", value: "47" }
            ]
        }
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningkelainanempedubayi\/create\/(\d+)/);
    ...
    // =========================
    // USIA DALAM BULAN CALCULATION & TEMPLATE SELECTION
    // =========================
    function calculateAgeInMonths(patientData) {
        // Cek apakah ada field khusus untuk bulan di patientData
        if (patientData.usia_bulan !== undefined && patientData.usia_bulan !== null) {
            return parseFloat(patientData.usia_bulan);
        }

        if (patientData.bulan !== undefined && patientData.bulan !== null) {
            return parseFloat(patientData.bulan);
        }

        // Fallback: konversi dari usia (tahun) ke bulan
        const usiaTahun = parseFloat(patientData.usia) || 0;
        return usiaTahun * 12;
    }
    ...
    function checkAgeAndSelectTemplate() {
        ...
        const ageInMonths = calculateAgeInMonths(patientData);
        const gender = patientData.gender || '?';
        const nama = patientData.nama || 'Pasien';
        const usiaTahun = patientData.usia || '?';

        console.log(`📋 Data Pasien: ${nama}, ${gender}, ${usiaTahun} tahun (${ageInMonths.toFixed(2)} bulan)`);
        console.log(`🔢 Usia dalam bulan: ${ageInMonths.toFixed(2)}`);

        // Validasi: pastikan usia dalam bulan valid
        if (isNaN(ageInMonths) || ageInMonths < 0) {
            console.error('❌ Usia dalam bulan tidak valid');
            showErrorPopup(
                'Data usia tidak valid',
                `Pasien: ${nama}\nUsia: ${usiaTahun} tahun\n\nSistem tidak dapat menghitung usia dalam bulan. Pastikan data pasien lengkap.`
            );
            return;
        }

        // Tentukan template berdasarkan usia dalam bulan
        let templateKey;
        let ageRange;
        ...
        const selectedTemplate = empeduBayiTemplates[templateKey];
        ...
        console.log(`✅ Skrining Kelainan Empedu Bayi berlaku untuk pasien ini (${ageInMonths.toFixed(2)} bulan)`);

        // Langsung fill form dengan template yang dipilih
        fillEmpeduBayiForm(patientData, selectedTemplate, ageRange, ageInMonths);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('empeduBayiProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'empeduBayiProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('empeduBayiProgress');
        if (progressBar) {
    ...
    // =========================
    function fillEmpeduBayiForm(patientData, template, ageRange, ageInMonths) {
        console.log(`🚀 Memulai auto-fill Skrining Kelainan Empedu Bayi (${ageRange})...`);

        // Tampilkan progress
        showProgress(`Mengisi form Empedu Bayi (${ageRange})...`);
            ...
            setTimeout(() => {
                ...
                if (index === template.actions.length - 1) {
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            hideProgress();
                            showCompletionPopup(patientData, ageRange, ageInMonths);
                        }, 1000);
                    }, 800);
                }
    ...
    // COMPLETION POPUP
    // =========================
    function showCompletionPopup(patientData, ageRange, ageInMonths) {
        const overlay = document.createElement('div');
        ...
        popup.innerHTML = `
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                <strong>Usia dalam Bulan:</strong><br>
                • ${ageInMonths.toFixed(2)} bulan<br>
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Kelainan Empedu Bayi telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('empeduBayiProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Empedu Bayi telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Kelainan Empedu Bayi loaded (2 templates: <1bulan, >1bulan)');
