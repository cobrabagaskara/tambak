(function() {
    ...
    // =========================
    // TEMPLATE SKRINING DIABETES MELITUS
    // =========================
    const diabetesTemplate = {
        metadata: {
            url: "https://cirebon.epuskesmas.id/skriningdiabetesmelitus/create/",
            skrining: "diabetes_melitus",
            judul: "Formulir Skrining Diabetes Melitus",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
                notes: "Template Skrining Diabetes Melitus untuk semua gender dan usia"
            }
        },
        actions: [
            { type: "setRadio", name: "anamnesis[Sering lapar]", value: "0" },
            { type: "setRadio", name: "anamnesis[Sering haus]", value: "0" },
            { type: "setRadio", name: "anamnesis[Sering BAK/mengompol]", value: "0" },
            { type: "setRadio", name: "anamnesis[Penurunan Berat Badan 2-6 minggu terakhir]", value: "0" },
            { type: "setRadio", name: "anamnesis[Riwayat Pribadi Diabetes Melitus]", value: "0" },
            { type: "setRadio", name: "anamnesis[Riwayat Keluarga Diabetes Melitus]", value: "0" },
            { type: "setRadio", name: "anamnesis[Riwayat Merokok]", value: "0" },
            { type: "setRadio", name: "anamnesis[Riwayat Minum alkohol/Merokok di keluarga]", value: "0" },
            { type: "setRadio", name: "anamnesis[Kebiasaan makan manis]", value: "1" },
            { type: "setRadio", name: "anamnesis[Aktifitas fisik setiap hari]", value: "1" },
            { type: "setRadio", name: "anamnesis[Istirahat cukup]", value: "1" },
            { type: "setRadio", name: "anamnesis[Kurang Makan Buah dan Sayur]", value: "0" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningdiabetesmelitus\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Diabetes Melitus berlaku untuk semua pasien`);

        // Langsung fill form
        fillDiabetesForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('diabetesProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'diabetesProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('diabetesProgress');
        if (progressBar) {
    ...
    // FILL FORM
    // =========================
    function fillDiabetesForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Diabetes Melitus...');

        // Tampilkan progress
        showProgress('Mengisi form Diabetes Melitus...');

        // Auto-fill sesuai template
        diabetesTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === diabetesTemplate.actions.length - 1) {
                    setTimeout(() => {
                        hideProgress();
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL DIABETES MELITUS SELESAI
            </h2>
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Diabetes Melitus telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('diabetesProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Diabetes Melitus telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Diabetes Melitus loaded');
