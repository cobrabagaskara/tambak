(function() {
    ...
    // =========================
    // TEMPLATE SKRINING MALARIA
    // =========================
    const malariaTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningmalaria/create/",
            skrining: "malaria",
            judul: "Skrining Malaria",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            { type: "setRadio", name: "anamnesis[q1]", value: "0" },
            { type: "setRadio", name: "anamnesis[q2]", value: "0" },
            { type: "setRadio", name: "anamnesis[q3]", value: "0" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningmalaria\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Malaria berlaku untuk semua pasien`);

        // Langsung fill form
        fillMalariaForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('malariaProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'malariaProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('malariaProgress');
        if (progressBar) {
    ...
    // =========================
    function fillMalariaForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Malaria...');

        // Tampilkan progress
        showProgress('Mengisi form Malaria...');

        // Auto-fill sesuai template
        malariaTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === malariaTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                • 3 field radio telah terisi otomatis<br>
                • Semua jawaban: <span style="color: #27ae60; font-weight: bold;">"Tidak"</span><br>
                • Skor: <span style="color: #9b59b6; font-weight: bold;">"1"</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Malaria telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('malariaProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Malaria telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Malaria loaded (4 field, tanpa kondisi)');
