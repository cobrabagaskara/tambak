(function() {
    ...
    // =========================
    // TEMPLATE SKRINING KANKER KOLOREKTAL
    // =========================
    const kolorektalTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningkankerkolorektal/create/",
            skrining: "kanker_kolorektal",
            judul: "Formulir Skrining Kanker Kolorektal",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
                notes: "Template Skrining Kanker Kolorektal untuk semua gender dan usia"
            }
        },
        actions: [
            { type: "setRadio", name: "riwayat_bab", value: "0" },
            { type: "setRadio", name: "riwayat_polip", value: "0" },
            { type: "setRadio", name: "riwayat_reseksi", value: "0" },
            { type: "setRadio", name: "riwayat_kanker_kolorektal", value: "0" },
            { type: "setRadio", name: "riwayat_merokok", value: "0" },
            { type: "setRadio", name: "kesediaan_colok_dubur", value: "0" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningkankerkolorektal\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Kanker Kolorektal berlaku untuk semua pasien`);

        // Langsung fill form
        fillKolorektalForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('kolorektalProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'kolorektalProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('kolorektalProgress');
        if (progressBar) {
    ...
    // FILL FORM
    // =========================
    function fillKolorektalForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Kanker Kolorektal...');

        // Tampilkan progress
        showProgress('Mengisi form Kanker Kolorektal...');

        // Auto-fill sesuai template
        kolorektalTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === kolorektalTemplate.actions.length - 1) {
                    setTimeout(() => {
                        hideProgress();
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL KANKER KOLOREKTAL SELESAI
            </h2>
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Kanker Kolorektal telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('kolorektalProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Kanker Kolorektal telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Kanker Kolorektal loaded');
