(function() {
    ...
    // =========================
    // TEMPLATE SKRINING RISIKO STROKE
    // =========================
    const strokeTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningrisikostroke/create/",
            skrining: "risiko_stroke",
            judul: "Skrining Risiko Stroke",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
                notes: "Template untuk semua pasien"
            }
        },
        actions: [
            { type: "setRadio", name: "kurang_aktifitas_fisik", value: "0" },
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningrisikostroke\/create\/(\d+)/);
    ...
    // REPLACE FUNCTION setValue YANG LAMA DENGAN INI:
function setValue(name, value) {
    // HAPUS SPASI DI AWAL/ AKHIR VALUE (CRITICAL!)
    const cleanValue = value.trim();

    const inputSelector = `input[name="${name}"], textarea[name="${name}"], select[name="${name}"]`;
    waitForElement(inputSelector, (input) => {
        if (input) {
            // Untuk dropdown CARTA - handle khusus
            if (name === "carta" && input.tagName === "SELECT") {
                // Cari option yang match (case-insensitive, ignore spasi)
                let optionFound = null;
                for (let i = 0; i < input.options.length; i++) {
                    const opt = input.options[i];
                    if (opt.value.trim() === cleanValue ||
                        opt.getAttribute('data-val')?.includes(cleanValue)) {
                        optionFound = opt;
                        break;
                    }
                }
                ...
                if (optionFound) {
                    input.value = optionFound.value;
                    input.selectedIndex = optionFound.index;

                    console.log(`✓ CARTA dropdown set ke: value="${optionFound.value}", text="${optionFound.text}"`);
                } else {
                    console.warn(`⚠️ Option tidak ditemukan untuk CARTA value="${cleanValue}"`);
                    // Fallback: pilih option pertama yang valid
                    if (input.options.length > 1) {
                        input.value = input.options[1].value; // Skip "Pilih CARTA"
            ...
            // TRIGGER EVENT LENGKAP (untuk bypass validasi custom)
            const events = [
                'focus',
                'input',
                'change',
            ...
            // FORCE REPAINT untuk pastikan UI update
            setTimeout(() => {
            ...
            console.log(`✓ Value set: ${name} = "${cleanValue}" (original: "${value}")`);
        }
    });
}

// REPLACE FUNCTION fillStrokeForm YANG LAMA DENGAN INI:
function fillStrokeForm(patientData) {
    console.log('🚀 Memulai auto-fill Skrining Risiko Stroke...');

    // Tampilkan progress
    showProgress('Mengisi form Risiko Stroke...');

    // Auto-fill sesuai template
    strokeTemplate.actions.forEach((action, index) => {
        setTimeout(() => {
            ...
            // Setelah action terakhir, trigger validation DAN SET ULANG CARTA
            if (index === strokeTemplate.actions.length - 1) {
                setTimeout(() => {
                    triggerFormValidation();

                    // 🔑 CRITICAL: SET ULANG CARTA SETELAH VALIDATION (override reset oleh halaman)
                    setTimeout(() => {
                        console.log('🔄 Memastikan nilai CARTA tetap "<5%"...');
                        setValue("carta", "1"); // Force set ke "<5%"
                    }, 400);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Risiko Stroke berlaku untuk semua pasien`);

        // Langsung fill form
        fillStrokeForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('strokeProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'strokeProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('strokeProgress');
        if (progressBar) {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL RISIKO STROKE SELESAI
            </h2>
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                <strong>Total Pertanyaan:</strong><br>
                • 4 field radio telah terisi otomatis<br>
                • Semua faktor risiko: <span style="color: #27ae60; font-weight: bold;">"Tidak"</span><br>
                • CARTA: <span style="color: #9b59b6; font-weight: bold;">"1"</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Risiko Stroke telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('strokeProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Risiko Stroke telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Risiko Stroke loaded (5 field, tanpa kondisi)');
