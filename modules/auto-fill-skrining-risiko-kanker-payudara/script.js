(function() {
    ...
    // =========================
    // TEMPLATE SKRINING RISIKO KANKER PAYUDARA
    // =========================
    const kankerPayudaraTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningkankerpayudara/create/",
            skrining: "risiko_kanker_payudara",
            judul: "Skrining Risiko Kanker Payudara",
            conditions: {
                gender: "Perempuan",
                minAge: null,
                maxAge: null,
                notes: "Template hanya untuk pasien Perempuan"
            }
        },
        actions: [
            { type: "setRadio", name: "merokok", value: "Tidak" },
            { type: "setRadio", name: "kurang_aktifitas_fisik", value: "Tidak" },
            { type: "setRadio", name: "gula_berlebih", value: "Tidak" },
            { type: "setRadio", name: "garam_berlebih", value: "Tidak" },
            { type: "setRadio", name: "lemak_berlebih", value: "Tidak" },
            { type: "setRadio", name: "kurang_buah_sayur", value: "Ya" },
            { type: "setRadio", name: "konsumsi_alkohol", value: "Tidak" },
            { type: "setRadio", name: "hasil_sadanis", value: "290084006" },
            { type: "setValue", name: "kesimpulan", value: "Normal" }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningkankerpayudara\/create\/(\d+)/);
    ...
    // =========================
    // GENDER VALIDATION & EXECUTION
    // =========================
    function validateGenderAndFillForm() {
        ...
        if (gender !== "Perempuan") {
            console.log(`🚫 Skrining Risiko Kanker Payudara tidak berlaku untuk ${gender}`);
            showErrorPopup(
                `Skrining Risiko Kanker Payudara hanya untuk Perempuan`,
        ...
        console.log('✅ Skrining Risiko Kanker Payudara berlaku untuk pasien ini');
        fillKankerPayudaraForm(patientData);
    }

    // =========================
    // ERROR POPUP (BUKAN PEREMPUAN)
    // =========================
    function showErrorPopup(title, message) {
        ...
        popup.innerHTML = `
            ...
            <div style="margin-top: 20px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #7f8c8d;">
                ℹ️ Skrining Risiko Kanker Payudara khusus untuk pasien perempuan.
            </div>
        `;
    ...
    function showProgress(message) {
        const existing = document.getElementById('kankerPayudaraProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'kankerPayudaraProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('kankerPayudaraProgress');
        if (progressBar) {
    ...
    // =========================
    function fillKankerPayudaraForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Risiko Kanker Payudara...');

        // Tampilkan progress
        showProgress('Mengisi form Kanker Payudara...');

        // Auto-fill sesuai template
        kankerPayudaraTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === kankerPayudaraTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL KANKER PAYUDARA SELESAI
            </h2>
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                • 9 field telah terisi otomatis<br>
                • Hasil Sadanis: <span style="color: #9b59b6; font-weight: bold;">290084006</span><br>
                • Kesimpulan: <span style="color: #27ae60; font-weight: bold;">Normal</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Risiko Kanker Payudara telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('kankerPayudaraProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Kanker Payudara telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Risiko Kanker Payudara loaded (Gender Validation: Perempuan Only)');

    // Tunggu DOM ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', validateGenderAndFillForm);
    } else {
        setTimeout(validateGenderAndFillForm, 500);
    }
