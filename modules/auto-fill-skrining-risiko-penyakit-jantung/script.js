(function() {
    ...
    // =========================
    // TEMPLATE SKRINING RISIKO PENYAKIT JANTUNG
    // =========================
    const jantungTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningrisikojantung/create/",
            skrining: "risiko_jantung",
            judul: "Skrining Risiko Penyakit Jantung",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            { type: "setRadio", name: "SkriningJantung[merokok]", value: "0" },
            { type: "setRadio", name: "SkriningJantung[kurang_aktifitas_fisik]", value: "0" },
            { type: "setRadio", name: "SkriningJantung[gula_berlebih]", value: "0" },
            { type: "setRadio", name: "SkriningJantung[garam_berlebih]", value: "0" },
            { type: "setRadio", name: "SkriningJantung[lemak_berlebih]", value: "0" },
            ...
            { type: "setValue", name: "SkriningJantung[carta]", value: " <5% " }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skriningrisikojantung\/create\/(\d+)/);
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Risiko Penyakit Jantung berlaku untuk semua pasien`);

        // Langsung fill form
        fillJantungForm(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('jantungProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'jantungProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('jantungProgress');
        if (progressBar) {
    ...
    // =========================
    function fillJantungForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Risiko Penyakit Jantung...');

        // Tampilkan progress
        showProgress('Mengisi form Risiko Jantung...');

        // Auto-fill sesuai template
        jantungTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === jantungTemplate.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <h2 style="color: #27ae60; margin-bottom: 10px; font-size: 24px; font-weight: 700;">
                ✅ AUTO-FILL RISIKO JANTUNG SELESAI
            </h2>
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px; margin: 20px 0; text-align: left; color: #555; line-height: 1.6;">
                ...
                • 2 jawaban "Ya" (Kurang Makan Sayur, Kardiovaskuler Rujuk RS)<br>
                • 7 jawaban "Tidak"<br>
                • CARTA: <span style="color: #9b59b6; font-weight: bold;">&lt;5%</span>
            </div>
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Risiko Penyakit Jantung telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('jantungProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill Risiko Jantung telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Risiko Penyakit Jantung loaded (9 field, tanpa kondisi)');
