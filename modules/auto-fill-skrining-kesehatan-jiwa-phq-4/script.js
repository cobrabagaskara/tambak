(function() {
    ...
    // =========================
    // TEMPLATE SKRINING KESEHATAN JIWA (PHQ-4)
    // =========================
    const phq4Template = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skriningkesehatanjiwadewasalansia/create/",
            skrining: "kesehatan_jiwa_phq4",
            judul: "Skrining Kesehatan Jiwa Dewasa dan Lansia (PHQ-4)",
            conditions: {
                gender: "Semua",
                minAge: null,
                maxAge: null,
        ...
        actions: [
            { type: "setRadio", name: "12", value: "on" },
            { type: "setRadio", name: "13", value: "on" },
            { type: "setRadio", name: "14", value: "on" },
            { type: "setRadio", name: "15", value: "on" }
        ]
    };

    // =========================
    // HELPERS - ENHANCED & FLEXIBLE
    // =========================
    function getPatientIdFromUrl() {
        const url = window.location.href;
        const match = url.match(/\/skriningkesehatanjiwadewasalansia\/create\/(\d+)/);
    ...
    // ENHANCED: Flexible radio button setter dengan multiple selector strategies
    function setRadio(name, value) {
        // Strategy 1: Try standard selector
        let radioSelector = `input[name="${name}"][value="${value}"]`;
        let radio = document.querySelector(radioSelector);

        // Strategy 2: If not found, try with quotes escaped
        if (!radio) {
            radioSelector = `input[name='${name}'][value='${value}']`;
            radio = document.querySelector(radioSelector);
        }

        // Strategy 3: If still not found, try finding by name only
        if (!radio) {
            const radiosByName = document.querySelectorAll(`input[name="${name}"]`);
            if (radiosByName.length > 0) {
                // Pick the first one or match by value if possible
                radio = Array.from(radiosByName).find(r => r.value === value) || radiosByName[0];
                radioSelector = `input[name="${name}"] (fallback)`;
            }
        }

        // Strategy 4: If still not found, try finding by value only
        if (!radio) {
            const radiosByValue = document.querySelectorAll(`input[value="${value}"]`);
            if (radiosByValue.length > 0) {
                radio = radiosByValue[0];
                radioSelector = `input[value="${value}"] (fallback)`;
            }
        }

        // If found, set it
        if (radio) {
            console.log(`✓ Found radio with selector: ${radioSelector}`);

            // Check if already checked
            if (radio.checked) {
                console.log(`✓ Radio already checked: ${name} = ${value}`);
                return;
            }
            ...
            // Trigger parent/container event
            ...
            console.error(`❌ Radio NOT FOUND: name="${name}", value="${value}"`);
            console.log('Available radio buttons on page:');
            const allRadios = document.querySelectorAll('input[type="radio"]');
            allRadios.forEach((r, i) => {
                console.log(`  ${i + 1}. name="${r.name}", value="${r.value}"`);
            });
        }
    }

    // ENHANCED: Force trigger form validation
    ...
    function validateAndFillForm() {
        ...
        console.log(`✅ Skrining Kesehatan Jiwa PHQ-4 berlaku untuk semua pasien`);

        // Langsung fill form
        fillPHQ4Form(patientData);
    }
    ...
    function showProgress(message) {
        const existing = document.getElementById('phq4Progress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'phq4Progress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('phq4Progress');
        if (progressBar) {
    ...
    // =========================
    // FILL FORM - WITH ENHANCED RADIO SETTING
    // =========================
    function fillPHQ4Form(patientData) {
        console.log('🚀 Memulai auto-fill Skrining Kesehatan Jiwa PHQ-4...');
        console.log('🔍 Scanning available radio buttons...');

        // Log semua radio buttons yang ada di halaman
        const allRadios = document.querySelectorAll('input[type="radio"]');
        ...
        // Tampilkan progress
        showProgress('Mengisi form PHQ-4...');

        // Auto-fill sesuai template
        phq4Template.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === phq4Template.actions.length - 1) {
                    setTimeout(() => {
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Kesehatan Jiwa PHQ-4 telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('phq4Progress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill PHQ-4 telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining Kesehatan Jiwa PHQ-4 v1.1 - FIXED loaded');
