(function() {
    ...
    // =========================
    // TEMPLATE SKRINING GDS-4
    // =========================
    const gdsTemplate = {
        meta: {
            url: "https://cirebon.epuskesmas.id/skrining/gds/create/",
            skrining: "gds_4",
            judul: "Formulir Skrining Geriatric Depression Scale (GDS-4)",
            conditions: {
                gender: "Semua",
                minAge: 60,
                maxAge: null,
                notes: "Template GDS-4 untuk lansia usia 60+ tahun"
            }
        },
        actions: [
            { type: "setRadio", name: "kepuasan_hidup", value: "0" },
            { type: "setRadio", name: "merasa_bosan", value: "1" },
            { type: "setRadio", name: "tidak_berdaya", value: "0" },
            { type: "setRadio", name: "merasa_tidak_berharga", value: "0" }
        ]
    };
    ...
    function getPatientIdFromUrl() {
        ...
        const match = url.match(/\/skrining\/gds\/create\/(\d+)/);
    ...
    function checkAgeAndFillForm() {
        ...
        if (usia < 60) {
            ...
            console.log(`🚫 Skrining GDS-4 tidak berlaku untuk usia ${usia} tahun`);
            showErrorPopup(
                `Skrining GDS-4 hanya untuk Lansia (60+ tahun)`,
        ...
        console.log(`✅ Skrining GDS-4 berlaku untuk pasien ini (${usia} tahun)`);
        fillGDSForm(patientData);
    }
    ...
    function showErrorPopup(title, message) {
        ...
        popup.innerHTML = `
            ...
            <div style="margin-top: 20px; padding: 10px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #7f8c8d;">
                ℹ️ Skrining GDS-4 khusus untuk lansia usia 60 tahun ke atas.
            </div>
        `;
    ...
    function showProgress(message) {
        const existing = document.getElementById('gdsProgress');
        if (existing) existing.remove();

        const progressBar = document.createElement('div');
        progressBar.id = 'gdsProgress';
    ...
    function hideProgress() {
        const progressBar = document.getElementById('gdsProgress');
        if (progressBar) {
    ...
    // FILL FORM
    // =========================
    function fillGDSForm(patientData) {
        console.log('🚀 Memulai auto-fill Skrining GDS-4...');

        // Tampilkan progress
        showProgress('Mengisi form GDS-4...');

        // Auto-fill sesuai template
        gdsTemplate.actions.forEach((action, index) => {
            setTimeout(() => {
                ...
                if (index === gdsTemplate.actions.length - 1) {
                    setTimeout(() => {
                        hideProgress();
    ...
    function showCompletionPopup(patientData) {
        ...
        popup.innerHTML = `
            ...
            <p style="color: #555; line-height: 1.7; margin-bottom: 25px; font-size: 14px;">
                Form Skrining Geriatric Depression Scale (GDS-4) telah terisi otomatis.<br>
    ...
    function cleanupAllUI() {
        ...
        // Hapus progress
        const progress = document.getElementById('gdsProgress');
        if (progress && progress.parentNode) progress.remove();

        console.log('🧹 Semua UI Auto Fill GDS-4 telah dibersihkan');
    }

    // =========================
    // INIT
    // =========================
    console.log('🟢 Auto Fill Skrining GDS-4 loaded');
